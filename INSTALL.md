...
- Sierra 2.1 or later
- A publically accessibly web server, with:
  - Python 3.4 or later
  - PostgreSQL 9.1 or later (but see below)

AutoHolds runs on a server, outside of the Sierra servers. You can choose to install it on the web server that runs your
web site, or on a separate server by itself.

AutoHolds doesn't specifically require PostgresSQL. Although we haven't tested it, it could also work with other
database systems that Django supports, like MySQL.
...
This user will run the AutoHolds application. If you're using  PostgreSQL, that name of this user needs to match the name of the role
you create next.


Create the autoHold database
----------------------------

These instructions for PostgreSQL and assume you have a working PostgreSQL system installed and running on the same
server as where you installing AutoHolds.

AutoHolds does not need an IP connection to PostgreSQL if PostgreSQL is running on the same server as AutoHolds.
AutoHolds will connect to PostgreSQL using the Unix-domain socket. This also means the autoholds user can connect to
PostgreSQL as the autoholds role without a password.

1. Connect to PostgreSQL as a superuser.
2. Create a role called autoholds.
3. Create a database called autoholds.
...
The following script would perform the steps above.
...
3. Enter the full name: AutoHolds App. Actually this can be anything you like.
...
7. Don't worry about any else. nothing else is used for SQL users.
8. Save the user.
...
1. Create a directory on your server to hold the AutoHolds app. We'll call this the install directory.
2. Make this directory owned by the autoholds user. Any directories leading up to the install directory must also be accessible by the autoholds user.
...
In addition to Python, AutoHolds requires the following Python packages.

- django 1.9.x
- requests 2.9 or later
- psycopg2 2.5 or later

If you are wanting to use a different database system for AutoHold, you will also need the Python package for that database system.
The [Django documentation](https://docs.djangoproject.com/en/1.9/ref/databases/) might help you to find out what package
you need.

The requirements.txt file in the install directory can be used together with the pip tool to do this.
...
1. After the `SIERRA_API` line:
1. In the `base_url` line, change `sierra-app-server.mylibrary.url` to the hostname of your Sierra application server.
1. In the `client_key` line, change `key` to the API key.
1. In the `client_secret` line, change `secret` to the API secret (password).
1. After the `SIERRA_SQL` line:
1. In the `host` line, change `sierra-db-server.mylibrary.url` to the hostname of your Sierra database server (not your Sierra application server).
1. In the `password` line, change `pass` to the password of the autoholds Sierra user.
...
1. In the `SECRET_KEY` line, change `CHANGE ME` to a long random string.  See the [Django documentation](https://docs.djangoproject.com/en/1.9/howto/deployment/checklist/#secret-key) for details.
...
This will create the necessary tables in the PostgreSQL database. The output should be something like this:
...
This is safe because there are actually no registrations in the AutoHolds app yet. So no holds will be placed.

1. From within the install directory, run the command: `python manage.py autoholds`.

This will establish establish a starting date and time for the AutoHolds app. Only bibs created from now will be
considered for AutoHolds.

This will test that the AutoHolds app can use the API key and secret to connect to Sierra.
...
1. Enter a username for yourself.
1. Enter your email address.
1. Enter a password for your AutoHolds Django user.
...
Note that this is only temporary. While the AutoHolds app may looks to be running, it is running insecurely. You might
properly deploy the AutoHolds app before allow access to it beyond yourself.
